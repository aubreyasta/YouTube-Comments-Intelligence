...
`DELETE /api/runs/{id}` stops a run at either point. Its conditional DELETE removes a `queued` run, so a run claimed a moment earlier is never deleted from under its thread: it falls through to `progress.cancel()` and is stopped properly instead.

`progress.cancel()` marks the row `failed`/`error` in the caller's thread, so the stream closes and the page reads "stopped" at once. The run's own thread is not interrupted, because nothing holds a handle to it. Instead `progress.publish()` is the checkpoint: its UPDATE matches only a still-running row, so the next stage write raises `progress.Cancelled` and `_execute` unwinds through its own handler. Every publish site is therefore a checkpoint at no cost, and a run whose row was deleted by a later overwrite stops the same way instead of computing to the end.

Two consequences follow from the thread outliving the row. The gap between checkpoints is a whole pipeline call, so a stopped run can keep computing for as long as one model call takes, and the slot frees immediately, so the next run may start while the stopped one is still finishing that call. `progress.finish()` carries the same `state` guard for the same reason: the last publish is several steps ahead of it, and without the guard a stop landing in that window would be overwritten and the run would read as `complete`.
